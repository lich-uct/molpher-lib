...
class InterlayAtom(wrappers.InterlayAtom):
    """

    This a specialized version of the `molpher.swig_wrappers.core.morphing.operators.InterlayAtom` proxy class.
    It implements some additional functionality for ease of use from Python.

    .. seealso:: `molpher.swig_wrappers.core.InterlayAtom`
    ...
    def morph(self):
        ret = super(InterlayAtom, self).morph()
        if ret:
            ret.__class__ = MolpherMol
        return ret

    def getOriginal(self):
        ret = super(InterlayAtom, self).getOriginal()
        if ret:
            ret.__class__ = MolpherMol
        return ret