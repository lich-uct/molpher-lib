...
class ContractBond(wrappers.ContractBond):
    """

    This a specialized version of the `molpher.swig_wrappers.core.morphing.operators.ContractBond` proxy class.
    It implements some additional functionality for ease of use from Python.

    .. seealso:: `molpher.swig_wrappers.core.ContractBond`
    ...
    def morph(self):
        ret = super(ContractBond, self).morph()
        if ret:
            ret.__class__ = MolpherMol
        return ret

    def getOriginal(self):
        ret = super(ContractBond, self).getOriginal()
        if ret:
            ret.__class__ = MolpherMol
        return ret