...
class AntidecoysFilter(TreeOperation):
    ...
    @staticmethod
    def get_manager():
        m = AntidecoysFilter.ComparisonManager()
        m.start()
        return m
    ...
    def eval_morph(data):
        if data:
            proxy, mol, idx, antifingerprint = data
    ...
    def __call__(self):
        if self.antifingerprint:
            ...
            proxy = self.manager.ComparisonProxy(mask)
            candidates = [(proxy, Chem.MolFromSmiles(mol.smiles), idx, self.antifingerprint) if mask[idx] else None for idx, mol in enumerate(tree.candidates)]
            pool = multiprocessing.Pool(THREADS)
            pool.map(func=self.eval_morph, iterable=candidates)
            pool.close()
            pool.join()

            print('Anti-decoys filter eliminated {0} molecules...'.format(proxy.get_counter()))
            tree.candidates_mask = proxy.get_mask()
